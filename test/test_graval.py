...
def test_encrypt_decrypt():
    ...
    _ = miner.initialize(seed)
    device_info = miner.get_device_info(0)
    print(f"Device info: {device_info} {device_info.__class__}")
...
def test_device_info_challenge():
    miner = Miner()
    device_count = miner.initialize(42)
...
def test_filesystem_challenge():
    ...
    for i in range(int(size / chunk_size)):
        ...
        miner_output = miner.process_filesystem_challenge(
            path, i * chunk_size, chunk_size
        )
